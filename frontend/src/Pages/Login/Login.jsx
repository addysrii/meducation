...
export default function Login() {
  ...
  const handleGoogleSuccess = async (tokenResponse) => {
    ...
    // We get access_token from useGoogleLogin, but wait: verifyIdToken in backend needs an id_token.
    // However, if we use flow: 'implicit' (default), we only get access token.
    // If we need id_token, we use useGoogleLogin with flow: 'implicit' doesn't give it properly, or we can use the credential from GoogleLogin component. 
    // To use useGoogleLogin and get an ID token easily, we do this:
  };
  ...
  const googleLoginHandler = useGoogleLogin({
    onSuccess: async (tokenResponse) => {
        ...
        setErr('');
        try {
           const response = await fetch(`${api}/api/${userType}/google-login`, {
             method: 'POST',
           ...
           if (!response.ok) {
              setErr(responesData.message || "Google Login failed");
           ...
           setErr("Network error during Google login");
        } finally {
           setGoogleLoading(false);
        }
    },
          ...
          <div className="bg-white rounded-[2.5rem] border-4 border-slate-900 shadow-[12px_12px_0px_0px_#0f172a] p-8 md:p-10">
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-5">
              ...
              <motion.button
                ...
                whileHover={!(loading || googleLoading) ? { y: -4, boxShadow: "6px 6px 0px 0px #0f172a" } : {}}
              ...
              <div className="flex items-center gap-4 my-2">
                ...
                <span className="font-black text-xs uppercase tracking-widest text-slate-400">OR</span>
                <div className="flex-1 h-1 bg-slate-100 rounded-full"></div>
              </div>
              ...
              <motion.button
                ...
                disabled={loading || googleLoading}
                whileHover={!(loading || googleLoading) ? { y: -4, boxShadow: "4px 4px 0px 0px #0f172a" } : {}}
                whileTap={!(loading || googleLoading) ? { scale: 0.95 } : {}}
                className={`w-full py-4 rounded-xl font-black text-lg flex items-center justify-center gap-3 border-4 border-slate-900 shadow-[4px_4px_0px_0px_#0f172a] transition-colors cursor-pointer ${googleLoading ? 'bg-slate-200 text-slate-400' : 'bg-white text-slate-900 hover:bg-slate-50'}`}
              >
