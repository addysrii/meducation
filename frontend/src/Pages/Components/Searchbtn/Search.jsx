...
function Search() {
  ...
  const handleEnroll = async (courseName, id) => {
    let check = await fetch(`/api/course/${courseName}/${id}/verify/student/${ID}`, {
        method: "POST", headers: { "Content-Type": "application/json" }
    });
    const res = await check.json();
    if(res.statusCode === 200){
      const dataReq = await fetch(`/api/payment/course/${id}/${courseName}`, {
        method: "POST", headers: { "Content-Type": "application/json" },
      ...
      const options = {
        ...
        handler: async (response) => {
          ...
          if (verifyRes.statusCode === 200) {
            try {
              let enrolRes = await fetch(`/api/course/${courseName}/${id}/add/student/${ID}`, {
                  method: "POST", headers: { "Content-Type": "application/json" }
              });
